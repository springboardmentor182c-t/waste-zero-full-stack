...
export const PickupService = {
  async createPickup(data) {
    try {
      ...
      console.error("❌ PickupService.createPickup Error:", error); // ✅ full stack trace
  ...
  async updatePickupStatus(pickupId, status) {
    // try {
    //   return await Pickup.findByIdAndUpdate(pickupId, { status }, { new: true });
    // } catch (error) {
    ...
    // }
     try {
      ...
      console.error("❌ PickupService.updatePickupStatus Error:", error);
      throw new Error("Failed to update pickup status");
    }
